...
namespace Server.Orchestration
{
    ...
    public class TransactionOrcWrite : ITransactionOrcWrite
    {
        ...
        public async Task<ResultSqlActionData<TransactionActionBasic>> TransactionActionUpdate(TransactionActionBasic transactionActionBasic)
        {
            using (TransactionBL transactionBL = transactionBLFactory.Create())
            {
                return await transactionBL.TransactionActionUpdate(transactionActionBasic);
            }
        }
    }
}
